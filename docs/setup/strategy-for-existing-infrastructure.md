...
| 3 — Secrets Manager | Create secrets, attach policies | Consume the prefix. `@lp-ai/config` already supports this |
...
3. **Treat secrets as inputs, not as something the app fetches by name.** Right now `@lp-ai/config` fetches by name. That's fine, but document the *contract*: "given a secret at path `lp-internal/database/url` containing JSON `{ url: string }`, the app will work." The client then wires our contract into their secret-naming conventions without us having to know what they are.
